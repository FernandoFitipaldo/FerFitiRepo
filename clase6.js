...
function renderizandoElementos() {
    ...
    const noticias= document.querySelector("main");

    listadoNoticias.forEach(listadoNoticias => {
        noticias.innerHTML +=`<article>
        <h2>${listadoNoticias.titulo}</h2>
        <img src="${listadoNoticias.foto}" alt="imagen">
        <p>${listadoNoticias.epigrafe}</p>
    </article>`
    }
        )
